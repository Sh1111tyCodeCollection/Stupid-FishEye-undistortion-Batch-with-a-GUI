...
function runTillTheEnd(){
 $.post( "/transferimage", { image: distorter.getSrc(), file_name: tempImg })
  .done(function( data ) {
    if (allImgs.length>0){

    setTimeout(function(){ runTillTheEnd(); }, 100);
    }else{
    alert("it ends here bro");
    }
  });

tempImg= allImgs.pop();
distorter.setImage("data/"+tempImg);
}
...
setTimeout(function(){ runTillTheEnd(); }, 1000);
